...
public class TidalLockRenderer extends EntityRenderer<TidalLockEntity> {
    ...
    public void render(TidalLockEntity entity, float yaw, float partialTicks, PoseStack poseStack, MultiBufferSource multiBufferSource, int light) {
        poseStack.pushPose();
        ...
        this.model.renderToBuffer(poseStack, vertexconsumer, light, OverlayTexture.NO_OVERLAY, RenderHelper.colorf(0f, 0f, 0f, 1f));
    ...
    @Override
    public ResourceLocation getTextureLocation(TidalLockEntity pEntity) {
        return NOCULL;
    }

    public static class IceTombModel extends EntityModel<TidalLockEntity> {
        ...
        @Override
        public void setupAnim(TidalLockEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        }
    }
}
