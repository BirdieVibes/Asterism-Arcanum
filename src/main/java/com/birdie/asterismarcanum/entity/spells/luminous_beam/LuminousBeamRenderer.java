...
public class LuminousBeamRenderer extends EntityRenderer<LuminousBeamProjectile> {
    ...
    public static final RenderType LUMINOUS_BEAM = RenderType.create(
            "luminous_beam", DefaultVertexFormat.POSITION_COLOR,
    ...
    @Override
    public void render(LuminousBeamProjectile entity, float entityYaw, float pPartialTicks, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        double entityX = Mth.lerp(pPartialTicks, entity.xo, entity.getX());
        double entityY = Mth.lerp(pPartialTicks, entity.yo, entity.getY());
        double entityZ = Mth.lerp(pPartialTicks, entity.zo, entity.getZ());
        ...
        VertexConsumer buffer = pBuffer.getBuffer(LUMINOUS_BEAM);

        // 3. QUADRATIC PROGRESS & ROTATION
        float time = (float) entity.tickCount + pPartialTicks;
        ...
        for (int i = 0; i < BEAM_COUNT; i++) {
            ...
            // Raycast logic
            Vec3 beamDir = entity.calculateRayDirection(smoothLookVec, (float) Math.cos(angle) * currentSpread, (float) Math.sin(angle) * currentSpread, 1.0F);
            float dynamicLen = entity.getBeamLength(eyePos, beamDir, BEAM_LENGTH);
        ...
        if (linearProgress > 0.5F) {
            ...
            float coreLen = entity.getBeamLength(eyePos, smoothLookVec, BEAM_LENGTH);
    ...
    @Override
    public ResourceLocation getTextureLocation(LuminousBeamProjectile pEntity) {
